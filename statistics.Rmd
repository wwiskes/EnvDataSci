...
wChemData <- read_excel(system.file("extdata","SinkingCoveWaterChem.xlsx", package="iGIScData")) %>%
...
wChemTrib <- wChemData %>% filter(siteLoc %in% c("B", "F", "K", "W")) %>% mutate(siteType = "tributary")
wChemData <- bind_rows(wChemTrunk, wChemDrip, wChemTrib)
sites <- read_csv(system.file("extdata", "SinkingCoveSites.csv", package="iGIScData"))
wChem <- wChemData %>%
...
- There's also pretty clearly a difference among lithologies on the basis solutes, not surprising since limestone is much more soluble than sandstones. Similarly, analysis of variance confirms the obvious.
...
- So elevation could work similarly to geology.
```{r}
wChemData %>%
