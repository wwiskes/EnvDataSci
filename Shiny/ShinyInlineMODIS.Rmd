...
The code shown can also be used to create a Shiny web app by saving it as `app.r` in an RStudio Shiny app project. In terms of complexity of inputs, this app is pretty straightforward -- the fluidPage is pretty standard for the ui section.  The leaflet map is pretty straightforward (though getting the point symbology right took some experimentation.  There are also some other tricks that make it a bit more complicated, such as web scraping to download new data, and using `observe` as a way to observe some parameters to allow the current zoom level to stick. 

```{r MODISfires, warning=F, message=F, collapse=T}
library(shiny); library(sf); library(leaflet); library(tidyverse); library(lubridate)
